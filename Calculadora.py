...
class Calculadora(tk.Tk):
    ...
    def _creacion_componentes(self):
        ...
        entrada_frame = tk.Frame(self, width=320, height=50, bg='grey')
        entrada_frame.pack(side=tk.TOP)
        # Caja de Texto
        entrada = tk.Entry(entrada_frame, font=('currier', 18 , 'bold'),
        ...
        botones_frame = tk.Frame(self, width=350, height=370, bg='purple')
        botones_frame.pack()

        # Primer renglón
        # Botón limpiar
        boton_limpiar = tk.Button(botones_frame, text='CLEAR', width='32', height=3,
        ...
        boton_evaluar = tk.Button(botones_frame, text='=', width=10, height=3, bd=0, bg='#eee',
                                cursor='arrow', command=self._evento_evaluar)
        boton_evaluar.grid(row=4, column=3, padx=1, pady=1)

    def _evento_evaluar(self):
        # eval evalua la expresion de tipo str como una expresion aritemetica
        try:
            if self.expresion:
                resultado = str(eval(self.expresion))
                self.entrada_texto.set(resultado)
        except Exception as e:
            messagebox.showerror('Error', f'Ocurrio un erro: {e}')
